...
HWE$qvalue = p.adjust(HWE$P,method="fdr")
thr = HWE[HWE$qvalue < 0.05 ,]
thr = thr[which.max(thr$qvalue),]$P
...
chrX = map[map[,1] == 30,]
chrX = chrX[chrX[,4] < 137000000,]  # 137 million base pair   or 143451431 for GPPR143
...
Sex = read.table("clean-inds-GWA-data.fam")
Sex_new = read.table("/home/ulg/genan/mahmoud/Annalys/Final_Analysis/Pheno.txt",header=T)
rownames(Sex_new) = Sex_new[,2]
Sex_new = Sex_new[as.character(Sex[,2]),]

mat <- snpgdsGetGeno(genofile)
chrX_geno = mat[,as.numeric(rownames(chrX))]
rownames(mat) = Sex[,2]

discordant_sex = NULL
for(i in 1:nrow(mat))
{
	discordant_sex[i] = sum(chrX_geno[i,] == 1) / (ncol(chrX_geno) - sum(chrX_geno[i,] == 3))
}
...
egend("topright",col=c(rgb(0,0,1,1/4),rgb(1,0,0,1/4)),legend=c("Females","Males"), bty="n",pch = 19)
dev.off()
